...
get(Mode, RunType) ->
  BaseNames = [name, port, type, yaws_dir, ssd_dir],
  Options = case Mode of
    start  -> get_argument(get_function(BaseNames));
    stop   -> get_argument(get_function([node]));
    copy   -> get_argument(get_function([node | BaseNames]));
...
get_function(name) -> {name, fun ssd_argument:validate_and_convert_name/1};
get_function(port) -> {port, fun ssd_argument:validate_and_convert_port/1};
get_function(type) -> {type, fun ssd_argument:validate_and_convert_type/1};
get_function(node) -> {node, fun ssd_argument:validate_and_convert_node/1};
get_function(message) ->
  {message, fun ssd_argument:validate_and_convert_message/1};
get_function(yaws_dir) -> {yaws_dir, validate_dir_function("yaws")};
get_function(ssd_dir)  -> {ssd_dir,  validate_dir_function("ssd_argument")}.
...
validate_dir_function(BeamName) ->
  fun (Dir) ->
    File = Dir ++ "/ebin/" ++ BeamName ++ ".beam",
    case file:read_file_info(File) of
      {ok, _FileInfo}  -> Dir;
      {error, _Reason} -> {badarg, File ++ " does not exist."}
    end
  end.
